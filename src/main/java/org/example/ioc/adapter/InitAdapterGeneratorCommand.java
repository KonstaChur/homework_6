...
import org.example.command.log.LogUnprocessedMethodCommand;
import org.example.command.operations.UpdateObjectPropertyCommand;
...
public class InitAdapterGeneratorCommand implements Command {
    ...
    public void execute() {
        try {
            ...
            var source = String.format(
                    "package org.example.operations; public class %s implements %s { %s %s %s }",
                    className,
                    adapterInterface.getName(),
                    "private final org.example.objects.UObject uObject;",
            ...
            File sourceFile = new File(
                    root,
                    String.format("org/example/operations/%s.java", className));
            ...
            Class<?> cls = Class.forName(
                    String.format("org.example.operations.%s", className),
                    true,
                    classLoader);
    ...
    private String getConstructor(String className) {
        return String.format(
                "public %s(org.example.objects.UObject uObject) { this.uObject = uObject;}",
                className);
    }
    ...
    private String prepareGetMethodBody(String propertyName, Type methodReturnType) {
        return String.format(
                "return java.util.Optional.ofNullable(org.example.ioc.IoC.<%s>resolve(\"%s.get\", \"%s\", uObject));",
                methodReturnType
                        .getTypeName()
    ...
    private String prepareSetMethodBody(String propertyName, Parameter param) {
        return String.format(
                "org.example.ioc.IoC.<org.example.command.Command>resolve(\"%s.set\", \"%s\", uObject, %s).execute();",
                adapterInterface.getSimpleName(),
                propertyName,
                param.getName());
    }
    ...
    private String prepareUnprocessedMethodBody(String propertyName) {
        return String.format(
                "org.example.ioc.IoC.<org.example.command.Command>resolve(\"%s.unprocessed\", \"%s\", \"%s\").execute();",
                adapterInterface.getSimpleName(),
                adapterInterface.getSimpleName(),
                propertyName);
    }
